...
export class EffectEngine {
  ...
  private transEnv = 0; private transAt = 0;   // ENERGY_FB: transientpuls (bred onset) med avklingning
  private lockGood = 0; private lockBpmRef = 0; private lockRamp = 1; private transAct = 0; private trustLowSince = 0;   // lotus-porten (se LOCK_BEATS)
  ...
  private lastBassNoteMs = -1e9;
  private prevBassOnset = 0;
  ...
  private seenLooks = new Set<Mode>();     // MIX_V2 (6): effekter som valts sedan start
  ...
  render(frame: Frame): Uint8Array {
    ...
    const nowWall = Date.now();
        ...
        if (LIVE_BEAT || ENERGY_FB) {
          ...
          if (ENERGY_FB) {
            ...
            if (on >= ENERGY_FB_ONSET && pn - this.transAt >= ENERGY_FB_GAP_MS && on >= this.transEnv * Math.exp(-(pn - this.transAt) / LIVE_BEAT_MS)) { this.transEnv = on; this.transAt = pn; }
    ...
    if (SECTION_SWITCH) {
      ...
      if (SECTION_DYN_DB > 0 && lv !== 0 && secNow !== 'high') dynGain = Math.max(SECTION_DYN_FLOOR, Math.min(1, 1 + lv / SECTION_DYN_DB));   // latens egen referens
    ...
    { const bo = frame.onset?.bass ?? 0;
      if (bo >= 0.30 && bo - this.prevBassOnset >= 0.10 && now - this.lastBassNoteMs >= 90) { this.bassNoteIdx++; this.lastBassNoteMs = now; }
      this.prevBassOnset = bo; }
    ctx.sectionBars = frame.sectionBars ?? 0; ctx.bassline = frame.profile?.bassline ?? 0;
    ctx.bassNoteIdx = this.bassNoteIdx; ctx.bassNoteAge = Math.min(9, (now - this.lastBassNoteMs) / 1000);
