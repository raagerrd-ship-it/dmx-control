...
export function scheduleSave(cfg: EngineConfig, path = DEFAULT_PATH, delayMs = 500) {
  ...
  saveTimer = setTimeout(async () => {
    try {
      ...
      const { identify: _omit, flashUntil: _omit2, beat: _omit3, sectionEnergy: _omit4, liveFlashUntil: _omit5, liveHueHint: _omit6, smartDwellMs: _o7, beatPulse: _o8, ...persist } = cfg;
