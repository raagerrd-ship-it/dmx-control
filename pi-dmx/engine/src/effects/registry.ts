...
export const SECTION_POOLS: Record<string, Mode[]> = {
...
for (const [sec, list] of Object.entries(SECTION_POOLS)) for (const e of EFFECTS) if (list.includes(e.key)) { (e.section ??= []).includes(sec) || e.section.push(sec); }
