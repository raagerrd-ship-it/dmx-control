 ...
 *
 * MIRROR of the DMX Control analyser. Lotus keeps only the adapter below:
 * `AnalyserConfig` is mapped to the master's internal config shape and the
...
export class Analyser {
  ...
  setVirtualClock(ms: number | null) {
    // TEST-HARNESS SHIM (added for the offline A/B only; live Lotus never calls
    // this — virtualMs stays null in production). Mirrors DMX's jump-guarded
    // version so per-hop calls don't reset the time-based vote/conf anchors and
    // corrupt the tempo voting. Isolates the tempo ALGORITHM for comparison.
