...
const { cfg, buffers, resumeSeq } = workerData as WorkerData;
const ctrl = new Int32Array(buffers.ctrl);
const an = new Analyser(cfg as EngineConfig, { role: 'slow', split: buffers, resumeSeq });
...
function pollMessages(): void {
  ...
  for (;;) {
    ...
    else if (msg?.type === 'crash' && TEST_CRASH) throw new Error('DMX_SPLIT_TEST_CRASH: avsiktlig worker-krasch (stresstest)');
  }
}
