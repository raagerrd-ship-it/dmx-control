...
// Record-falt (Float64)
export const R_SEQ = 0, R_PERF = 1, R_WALL = 2, R_ENV = 3, R_BASS = 4, R_HIGH = 5, R_FLAGS = 6, R_HINT_MS = 7, R_VCLOCK = 8,
  R_SEC_N = 9, R_SEC_INT = 10, R_SEC_KICKS = 11, R_SEC_BREAK = 12, R_SEC_RMS2 = 13, R_SEC_CENT = 14, R_SEC_DT = 15, R_SEC_WALL = 16,
  R_DROPS = 17, R_ACTIVE = 18, R_BUILD = 19, R_SEC_SPEC0 = 20 /* ..27 */,
  R_TS = 28 /* Date.now() vid skrivning, for lagmatt over tradar */,
  R_FLAGCNT = 29 /* packade flaggraknare (se packFlagCounts): hur manga ganger varje flagga rests t.o.m. detta record */;
...
export const S_REC_SEQ = 0, S_BPM = 1, S_CONF = 2, S_PHASE_MS = 3, S_PHASE_CONF = 4, S_SECTION = 5, S_SEC_START = 6,
  S_SEC_INDEX = 7, S_SEC_TIER = 8, S_REP_SIM = 9, S_REP_AGO = 10, S_REP_SEC = 11, S_EXPECT_MS = 12, S_EXPECT_SRC = 13,
  S_PREV_SEC = 14, S_LVL_HIGH = 15,
  S_PROCESSED = 16, S_LAG_MS = 17, S_LAG_MAX = 18, S_BUSY_US = 19, S_BUSY_MAX_US = 20, S_SKIPPED = 21,
  S_LOST_FLAGS = 22 /* flaggor aterskapade ur raknarna efter tapp */;
...
/** Meddelanden main <-> worker (bara utanfor driftvagen). `crash` finns ENBART bakom DMX_SPLIT_TEST_CRASH=1. */
export type WorkerMsg = { type: 'stop' } | { type: 'crash' };
export interface WorkerData { cfg: unknown; buffers: SplitBuffers; resumeSeq: number /* senast lasta record (full seq) - 0 vid forsta start */ }
