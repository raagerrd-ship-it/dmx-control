...
const checks={
 firedCount: fired.length>=2,
 firedPlaces: fired.every((f)=>dropsA.some((d)=>Math.abs(d-f)<800)),
 midNotKnown: !mid.known,
 seekKnown: afterSeek.known, seekErr: seekError<350,
 seekFiredPlaces: seekFired.every((t)=>dropsA.some((d)=>Math.abs(d-t)<800)),
 gaplessId: gapless.songId===2, gaplessPos: gapless.positionMs>10000&&gapless.positionMs<16000,
 gaplessBoundary: gapless.lastBoundary==="igenk\u00e4nd l\u00e5t #2",
 shortId: shortPrev.songId===2, shortPos: shortPrev.positionMs>10000&&shortPrev.positionMs<16000,
 shortSongs: shortPrev.songs===2,
 drKnown: dr.known, drErr: driftError<250, drJump:(drifted.maxJump??0)<60,
 memBnotKnown: mem2.state().known===false, pastEnd: pastEnd.known===false,
 trim: afterTrim===beforeTrim, drop: afterDrop===beforeTrim-1};
console.log(Object.entries(checks).filter(([,v])=>!v).map(([k])=>k).join(", ")||"alla OK");
