...
vanta pa tjansten. Inga hemligheter i repot:
  PI_PASS=... [PI_HOST=192.168.4.1] python tools/deploy-dist.py [--dry] [--no-restart] [--conf evidens] [--env DMX_TEMPO_EVIDENCE=1 ...] [--remove /sokvag|glob ...] [fil ...]
--remove (2026-09-23): filer pa Pi:n som inte langre finns i bygget flyttas till <fil>.bak-<ts> (aldrig rm); ladan.py bar listan.
...
# --remove <sokvag|glob> (2026-09-23, rensningen): filer pa Pi:n som inte langre finns i bygget. Flyttas till
...
if REMOVES: print(f"tas bort (flyttas till .bak): {len(REMOVES)} monster: " + ' '.join(REMOVES))
if DRY: sys.exit(0)
...
for pat in REMOVES:
    ...
    rc, out, _ = sudo(f'for f in {pat}; do [ -e "$f" ] && mv "$f" "$f.bak-{ts}" && echo "$f"; done; true')
    got = [l for l in out.split() if l]; removed += got
    print(f"borttaget -> .bak-{ts}: {got or 'inget'}   ({pat})")
if ENVS:
    name = CONF or 'deploy'
