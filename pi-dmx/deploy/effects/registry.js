...
export const TOGGLE_POOL = EFFECTS.filter((e) => e.toggle).map((e) => e.key);
...
export function meetsRequirements(key, bpm, p) {
    ...
    if (!r)
        return true;
    if (r.minBpm !== undefined && bpm < r.minBpm)
        return false;
    if (r.needsPunch !== undefined && p.punch < r.needsPunch)
        return false;
    if (r.needsBass !== undefined && p.bass < r.needsBass)
        return false;
    if (r.needsBright !== undefined && p.bright < r.needsBright)
        return false;
    if (r.needsBeat !== undefined && p.beat < r.needsBeat)
        return false;
    return true;
}
