...
export class IntensityKnob extends EventEmitter {
    ...
    value;
    lastClkEdge = 0;
    dtState = 1; // KY-040 pull-up: vilar högt
    constructor(opts) {
        super();
        this.opts = opts;
    ...
    set(v) { this.value = Math.max(0, Math.min(1, v)); }
    ...
    spawn() {
        ...
        p.stdout.on("data", (b) => {
            ...
            while ((nl = buf.indexOf("\n")) >= 0) {
                ...
                if (!Number.isFinite(edge) || !Number.isFinite(off))
                    continue;
                if (off === this.opts.dt) {
                ...
                if (off !== this.opts.clk)
                    continue;
                // KY-040: läs bara fallande CLK → en detent per hakk.
                if (edge !== 2)
                    continue;
                const now = performance.now();
                if (now - this.lastClkEdge < debounceMs)
                    continue;
