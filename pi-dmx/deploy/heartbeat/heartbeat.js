...
export function pulseEnvelope(dt, amp, p) {
    if (dt < 0)
        return 0;
    const r = p.riseMs;
    const H = r > 0 ? r * p.riseHoldK : 0;
    if (dt <= H)
        return r > 0 ? amp * (1 - Math.exp(-dt / r)) : amp;
...
export function pulseAmpFor(idx, subdivLevel, accent, barShift) {
...
export class PulseTrack {
    t = new Float64Array(8);
    a = new Float64Array(8);
    idx = new Float64Array(8).fill(-1e9);
    pos = 0;
    push(idxKey, tMs, amp) {
        for (let i = 0; i < 8; i++)
            if (this.idx[i] === idxKey) {
                if (amp > this.a[i])
                    this.a[i] = amp;
                return;
            }
        this.t[this.pos] = tMs;
        this.a[this.pos] = amp;
        this.idx[this.pos] = idxKey;
        this.pos = (this.pos + 1) & 7;
    }
    clear() { this.idx.fill(-1e9); }
    ...
    valueAt(tDisp, p) {
        ...
        for (let i = 0; i < 8; i++) {
            if (this.idx[i] <= -1e9)
                continue;
            const v = pulseEnvelope(tDisp - this.t[i], this.a[i], p);
            if (v > out)
                out = v;
        }
        return out;
...
export function pulseSplit(onsetBoost, predicted, barAccent) {
...
export function trustRaw(confidence, hasBeat, lo, hi) {
...
export function trustSmooth(raw, prev, tickMs, upMs, downMs) {
...
export function ceilingFrom(shape, buildUp, buildUpGain, one, barAccentLift, levelVsHighDb, dynDb, dynFloor) {
    let ceil = shape * (1 + buildUp * buildUpGain);
    ceil += (1 - ceil) * one * barAccentLift;
    if (ceil > 1)
        ceil = 1;
    if (dynDb > 0 && levelVsHighDb < 0) {
        const g = 1 + levelVsHighDb / dynDb;
        ceil *= g > dynFloor ? g : dynFloor;
    }
    return ceil;
}
/** Komposition till energiform 0..1: tak × sektionsskala × förväntan × ((1−bd) + bd·pn), drop lyfter mot 1. (= gamla energyForm) */
export function composeEnergy(ceil, secScale, build, beatDepthEff, pn, dropBoost) {
    const bd = beatDepthEff;
    let energyForm = ceil * secScale * build * ((1 - bd) + bd * pn);
    if (dropBoost > 0)
        energyForm += dropBoost * (1 - energyForm);
    if (energyForm > 1)
        energyForm = 1;
    return energyForm;
}
/** Golv..1-mappning (output-lagrets första steg; ligger här tills OUTPUT bryts ut). */
export function toNormalized(energyForm, floorN) {
