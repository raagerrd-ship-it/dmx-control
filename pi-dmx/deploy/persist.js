...
export async function loadConfig(path = DEFAULT_PATH) {
    ...
    for (const p of [path, `${path}.bak`]) {
        try {
            ...
            if (cfg) {
                if (p !== path)
                    console.warn(`[persist] main config unreadable — restored from ${p}`);
                return cfg;
            }
...
export function scheduleSave(cfg, path = DEFAULT_PATH, delayMs = 500) {
    if (saveTimer)
