...
export function applyInputRouting(input) {
    ...
    spawn("sh", ["-c", `alsactl restore 0 -f /etc/alsa/codec-zero-${input}.state 2>/dev/null; ${sw}`], { stdio: "ignore" })
        .on("error", (e) => console.error("[audioInput] spawn:", e.message)); // annars kraschar ett spawn-fel hela root-processen
}
export async function startServer(deps, port = 80, tls) {
    const app = Fastify((tls ? { logger: false, https: tls } : { logger: false }));
    ...
    const identifyOne = (index, holdMs = 1500) => {
        stopIdentify();
        if (index < 0 || index >= deps.cfg.fixtures.length)
            return;
    ...
    const pendingOf = (c) => (c.__pending ??= new Map());
    /** Skicka ett state-paket med koalescering per typ (senaste vinner). */
    const sendState = (c, type, s) => {
        if (c.readyState !== 1)
            return;
        if ((c.bufferedAmount ?? 0) > HIGH_WATER) {
            pendingOf(c).set(type, s);
            return;
        }
        const p = pendingOf(c);
        if (p.size)
            p.delete(type); // vi skickar det färska nu
        c.send(s);
    };
    ...
    const broadcast = (payload) => {
        const p = payload ?? { type: "config", config: deps.cfg };
        const s = JSON.stringify(p);
        for (const c of app.websocketServer.clients)
            sendState(c, p.type ?? "config", s);
    };
    ...
    const captiveRedirect = (_req, reply) => reply.code(302).header("location", CAPTIVE_PORTAL).send();
    ...
    app.setNotFoundHandler((req, reply) => {
        if (req.method === "GET")
            return captiveRedirect(req, reply);
    ...
    app.get("/health", (_req, reply) => {
        if (deps.getHealthy())
            ...
            reply.code(503).send(deps.getFailReason?.() || "stale");
    });
    ...
    app.post("/api/config/import", async (req, reply) => {
        ...
        try {
            ...
            spawn("systemd-run", ["--unit=pi-dmx-restart", "--collect", "--quiet",
                ...
                .on("error", (e) => console.error("[import] restart spawn:", e.message)).unref();
            return reply.send({ imported: true, restarting: true });
        }
        catch (e) {
            logHealth("err", "config", `import misslyckades: ${e.message}`);
    ...
    const gitInfo = () => {
        try {
            ...
            return { error: e.message, repo: REPO };
        }
    };
    ...
    app.post("/update", async (_req, reply) => {
        ...
        try {
            ...
            up.on("error", (e) => console.error("[update] spawn:", e.message)); // ej krascha om systemd-run saknas
            up.unref();
            logHealth("info", "update", "OTA-uppdatering startad");
            return reply.send({ started: true });
        }
        catch (e) {
            logHealth("err", "update", `start misslyckades: ${e.message}`);
            return reply.code(500).send({ error: e.message });
        }
    });
    ...
    app.post("/update/rollback", async (_req, reply) => {
        try {
            ...
            up.on("error", (e) => console.error("[rollback] spawn:", e.message));
            up.unref();
            logHealth("warn", "update", "rollback startad");
            return reply.send({ started: true });
        }
        catch (e) {
            logHealth("err", "update", `rollback misslyckades: ${e.message}`);
            return reply.code(500).send({ error: e.message });
        }
    });
    ...
    app.post("/factory-reset", async (_req, reply) => {
        try {
            ...
            spawn("sh", ["-c", `mv -f ${CFG} ${CFG}.factory-${ts} 2>/dev/null; mv -f ${CFG}.bak ${CFG}.bak.factory-${ts} 2>/dev/null; systemctl restart audio-dmx-engine`], { detached: true, stdio: "ignore" })
                .on("error", (e) => console.error("[factory-reset] spawn:", e.message)).unref();
            logHealth("warn", "config", "fabriks-reset — motorn startas om");
            return reply.send({ started: true });
        }
        catch (e) {
            logHealth("err", "config", `fabriks-reset misslyckades: ${e.message}`);
            return reply.code(500).send({ error: e.message });
        }
    });
    // ---- WiFi / phone hotspot ------------------------------------------------
    ...
    app.post("/wifi/hotspot/connect", async (_req, reply) => {
        ...
        spawn("nmcli", ["con", "up", HOTSPOT_CON], { detached: true, stdio: "ignore" })
            .on("error", (e) => console.error("[wifi] spawn:", e.message)).unref();
    ...
    const frameTick = () => {
        ...
        if (frame.level > pkLevel)
            pkLevel = frame.level;
        if (frame.energy > pkEnergy)
            pkEnergy = frame.energy;
        if (frame.kick)
            pkKick = true;
        const bc = deps.cfg.beat;
        if (bc && bc.bpm > 40) {
            const idx = Math.floor((Date.now() - bc.anchorMs) / (60000 / bc.bpm));
            if (lastBeatIdx >= 0 && idx > lastBeatIdx)
                pkBeat = true;
            lastBeatIdx = idx;
        }
        else {
        ...
        const s = JSON.stringify({
            ...
            beatMul: r3(frame.beatMul), // hjärtslagets faktiska djup (diagnostik)
        ...
        for (const c of clients) {
            ...
            if ((cc.bufferedAmount ?? 0) > HIGH_WATER) {
                frameDrops++;
                continue;
            }
    ...
    app.register(async (f) => {
        f.get("/ws", { websocket: true }, (conn) => {
            ...
            if (!frameTimer) {
                subTick = 0;
                frameTimer = setInterval(frameTick, 25);
            }
            sock.on("message", (raw) => {
                try {
                    ...
                    if (msg.type === "setMood" && isMood(msg.value)) {
                        ...
                        const m = { slow: 30000, normal: 15000, fast: 8000 };
                        deps.cfg.smartDwellMs = m[msg.mode] ?? 15000;
                    }
...
function isMode(m) {
    return typeof m === "string" &&
        (m === "smart" || m === "blackout" || EFFECT_MAP.has(m));
}
const clamp01 = (x) => typeof x === "number" && x >= 0 && x <= 1 ? x : 0;
const VALID_PRESETS = ["rgb", "rgb7", "rgbw", "dimmer", "custom"];
const VALID_ROLES = ["r", "g", "b", "w", "dim", "strobe", "hazer", "uv", "blinder", "laser", "co2", "unused"];
/** Validate + normalize a fixtures[] patch. Returns null if any entry is bogus. */
function sanitizeFixtures(input) {
    const out = [];
    for (const raw of input) {
        ...
        if (!Number.isFinite(address) || address < 1 || address > 512)
            return null;
        if (!VALID_PRESETS.includes(preset))
            return null;
        let roles;
        if (preset === "custom") {
            if (!Array.isArray(r.roles) || r.roles.length === 0 || r.roles.length > 32)
                return null;
            roles = [];
        ...
        const bandsArr = Array.isArray(r.bands)
            ? [...new Set(r.bands.filter((b) => ["bass", "mid", "treble", "kick", "low"].includes(b)))]
            : undefined;
        // Per-lampa ljus-kalibrering: off/on klippta till 0..255. Släpps om båda 0.
        let cal;
        if (r.cal && typeof r.cal === "object") {
            const cr = r.cal;
            const clampByte = (x) => Math.max(0, Math.min(255, Math.floor(Number(x)) || 0));
            const off = clampByte(cr.off), on = clampByte(cr.on);
            // Per-färg-trösklar (valfria): bara med om satta (>0).
            const perCol = {};
            for (const k of ["onR", "onG", "onB", "onW"]) {
                const v = clampByte(cr[k]);
                if (v > 0)
                    perCol[k] = v;
            }
            if (off > 0 || on > 0 || Object.keys(perCol).length)
                cal = { off, on, ...perCol };
        }
        const fx = { name, address, preset, ...(roles ? { roles } : {}), ...(bandsArr?.length ? { bands: bandsArr } : {}), ...(cal ? { cal } : {}) };
