...
export class AudioCapture extends EventEmitter {
    ...
    static CLEAN_RESPAWNS = 2;
    static LADDER_MAX = 4;
    static HEALTHY_MS = 5000;
    static SAFE_RETRY_MS = 60_000;
    recoveries = 0;
    firstDataAt = 0;
    safeMode = false;
    respawnTimer = null;
    /** True när trappan är slut: ingen respawn-jakt, bara ett försök per minut. */
    get inSafeMode() { return this.safeMode; }
    constructor(opts) {
        super();
        this.opts = opts;
    ...
    recover() {
        ...
        this.lastDataAt = 0;
        if (this.proc)
            this.proc.kill("SIGKILL"); // 'exit'-handlern respawnar
        else
            this.spawnArecord();
    }
    /** Dödar en levande men tyst arecord. Respawn sker via 'exit'-handlern. */
    checkStall() {
        if (this.stopped || !this.proc || this.lastDataAt === 0)
            return;
        const gap = Date.now() - this.lastDataAt;
        if (gap < AudioCapture.STALL_MS)
            return;
        this.emit("stall", gap);
    ...
    scheduleRecovery() {
        if (this.stopped || this.respawnTimer)
            return;
        this.recoveries++;
        ...
        if (this.recoveries > AudioCapture.LADDER_MAX) {
            if (!this.safeMode) {
                this.safeMode = true;
                this.emit("safe", this.recoveries);
            }
    ...
    toMonoFloat32(buf) {
        ...
        if (this.opts.channels === 1) {
            ...
            if (this.balN >= this.opts.rate * 3) {
                ...
                if (setting === "auto") {
                    const cand = loud <= -60 ? this.autoPick // for tyst for att doma
                        : loud - quiet <= 12 ? "mix"
                            : dbL > dbR ? "left" : "right";
                    if (cand === this.autoCand)
                        this.autoVotes++;
                    else {
                        this.autoCand = cand;
                        this.autoVotes = 1;
