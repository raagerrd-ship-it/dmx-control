...
DECIMERINGSREGEL (2026-08-23): allt inuti `if (++bigCounter >= BIG_EVERY)` körs 125/s, inte 375/s. Varje tidskonstant därinne MÅSTE skalas — `aBandLvl` via `bigDt`, `oStep = 0.002·BIG_EVERY`, `PEAK_DECAY = 0.9993^BIG_EVERY` (rå 0.9993 gav τ ≈ 11 s i stället för 3.8 s). AGC:ns `envelope` seedas till `autoGainTarget` i konstruktorn, `resetGain()` OCH `setGainLock(true)` — noll ger desired = target/1e-4 och slår gainen i 20x-taket (ljuspump vid ingångsbyte).
